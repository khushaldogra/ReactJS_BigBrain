...
import { PlayerResultsBody, ResultsCard, ResultTitle, ResultInfo, GameResultsButton } from '../styledComponents/PlayerResults';
...
function QuestionResult({ json, idx }) {
  ...
  return (
    <ResultsCard color={color}>
      <ResultTitle correct={json.correct}>Question {idx + 1}: {json.correct === true ? "Correct" : "Wrong"}</ResultTitle>
      <ResultInfo>Time Spent: {timeSpent.toFixed(1)} seconds</ResultInfo>
      <ResultInfo>Correct Answers: {json.answerIds.join(', ')}</ResultInfo>
    </ResultsCard>
  )
}
